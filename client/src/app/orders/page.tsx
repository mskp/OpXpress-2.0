...
export const metadata: Metadata = {
  title: "OpXpress | My Orders",
};

type FetchOrdersReturnType = {
  success: boolean;
  orders: Order[];
};

async function fetchOrders(): Promise<FetchOrdersReturnType> {
  const { data } = await axios.get<FetchOrdersReturnType>(`/order`);
  return data;
}
 ...
 */
async function OrdersPage(): Promise<JSX.Element> {
  let data: FetchOrdersReturnType;
  ...
  if (data.orders?.length === 0)
    return <ErrorComponent message="No orders found" />;
